...
def delete_everything_inside_folder(folder):
    ...
    logger.info_log("All files and folders successfully deleted from '{}'".format(folder))
...
def import_source(import_identifier: str, source_dir: str = ".source"):
    ...
    if os.path.exists(source_dir) is False:
        logger.info_log("'{}' does not exist, creating directory".format(source_dir))
        os.makedirs(source_dir, exist_ok=True)

    if os.path.exists(full_path) is False:
        logger.info_log("'{}' does not exist, creating directory".format(full_path))
        os.makedirs(full_path, exist_ok=True)

    logger.info_log("Attempting to determine if '{}' is a git repository".format(full_path))
    try:
        _ = git.Repo(full_path).git_dir
        logger.warn_log("'{}' is a git repository".format(full_path))

        logger.info_log("Reset the repository to make sure there is no changes ...")
        git.Repo(full_path).git.reset('--hard')

        logger.info_log("Pulling the latest from the `master` branch ...")
        git.Repo(full_path).remotes.origin.pull()

    except git.exc.InvalidGitRepositoryError:
        logger.warn_log("'{}' is NOT a git repository".format(full_path))
        logger.info_log("Attempting to clean the folder ...".format(full_path))
        delete_everything_inside_folder(full_path)

        repo_url: str = 'git@github.com:NoX-Scripts-and-Things/{}.git'.format(import_identifier)
        logger.info_log("Attempting to clone the github repository '{}'".format(repo_url))
        git.Repo.clone_from(repo_url, full_path)

    logger.info_log("Successfully imported '{}'\n".format(import_identifier))
